...
public class OAuth2DingtalkAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        // 创建用户认证令牌
        Dify4jOAuth2AuthorizationToken dify4jAuthorization = Dify4jOAuth2AuthorizationToken.authenticated(userDetails.getUserId(), userDetails.getAuthorities());
    ...
    private void createUser(DingtalkUser dingtalkUser) {
        try {
            ...
            //从请求获取数据填充
            app.setName(newUser.getName()+"智能体");
            app.setMode(AppMode.AGENT_CHAT.getValue());
            app.setCreatedBy(newUser.getId());
            app.setUpdatedBy(newUser.getId());
