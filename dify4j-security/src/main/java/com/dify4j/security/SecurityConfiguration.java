...
import com.dify4j.security.captcha.CaptchaAuthenticationFilter;
import com.dify4j.security.crypto.Dify4jDelegatingPasswordEncoder;
import com.dify4j.security.crypto.PasswordEncoderType;
import com.dify4j.security.oauth2.authorization.*;
import com.dify4j.security.oauth2.authorization.dify.DifyAuthenticationConverter;
import com.dify4j.security.oauth2.authorization.dify.DifyAuthenticationProvider;
import com.dify4j.security.oauth2.exception.Difj4jAccessDeniedHandler;
import com.dify4j.security.oauth2.exception.Dify4jAuthenticationEntryPoint;
...
import com.dify4j.security.service.SecurityServiceImpl;
import com.dify4j.security.user.Dify4jUserDetailsService;
...
public class SecurityConfiguration {
    ...
    //=========== 自定义异常响应组件 ==========
    protected Dify4jAuthenticationEntryPoint authenticationEntryPoint;

    protected Difj4jAccessDeniedHandler accessDeniedHandler;
    ...
    protected void init() {
        ...
        if (this.passwordEncoder == null) {
            this.passwordEncoder = new Dify4jDelegatingPasswordEncoder();
        }

        // 初始化令牌定制器（如果未设置）
        if (this.tokenCustomizer == null) {
            this.tokenCustomizer = new Dify4jOAuth2TokenCustomizer();
        }
        ...
        if (registeredClientRepository == null) {
            registeredClientRepository = new Difj4jJdbcRegisteredClientRepository(jdbcTemplate);
        }
        ...
        // 初始化自定义异常响应组件
        authenticationEntryPoint = new Dify4jAuthenticationEntryPoint(objectMapper);
        accessDeniedHandler = new Difj4jAccessDeniedHandler(objectMapper);
        ...
        if (authorizationService == null) {
            authorizationService = new Dify4jJdbcOAuth2AuthorizationService(jdbcTemplate, registeredClientRepository);
        }
     ...
     * @return 认证入口点实例
     */
    public Dify4jAuthenticationEntryPoint getAuthenticationEntryPoint() {
     ...
     * @return 访问拒绝处理器实例
     */
    public Difj4jAccessDeniedHandler getAccessDeniedHandler() {
    ...
    public static class Builder {
        ...
        private void validate() {
            // 验证必需的组件
            if (userService == null) {
                throw new Dify4jException("用户服务未设置");
            }
            if (jdbcTemplate == null) {
                throw new Dify4jException("JDBC模板未设置");
            }
            if (jwtEncoder == null) {
                throw new Dify4jException("JWT编码器未设置");
            }
            if (idmService == null) {
                throw new Dify4jException("身份服务未设置");
            }
        }
